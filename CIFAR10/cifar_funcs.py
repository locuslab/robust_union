...
def pgd_l2(model, X, y, epsilon=0.5, alpha=0.05, num_iter = 40, device = "cuda:0", restarts = 0):
...
def pgd_l1_topk(model, X,y, epsilon = 12, alpha = 0.05, num_iter = 40, k = 20, device = "cuda:1", restarts = 0):
...
def pgd_linf(model, X, y, epsilon=0.03, alpha=0.003, num_iter = 40, device = "cuda:0", restarts = 0):
...
def msd_v1(model, X,y, epsilon_l_inf = 0.03, epsilon_l_2= 0.5, epsilon_l_1 = 12, 
                alpha_l_inf = 0.003, alpha_l_2 = 0.05, alpha_l_1 = 0.05, num_iter = 50, device = "cuda:0"):
    delta = torch.zeros_like(X,requires_grad = True)
    max_delta = torch.zeros_like(X)
    max_max_delta = torch.zeros_like(X)
...
def pgd_all_old(model, X,y, epsilon_l_inf = 0.03, epsilon_l_2= 0.5, epsilon_l_1 = 12, alpha_l_inf = 0.003, alpha_l_2 = 0.05, alpha_l_1 = 0.05, num_iter = 50, device = "cuda:1"):
    percentage = [0,0,0]
    delta = torch.zeros_like(X,requires_grad = True)
    for t in range(num_iter):
        loss = nn.CrossEntropyLoss()(model(X + delta), y)
        ...
        delta_l_1  = temp_data + alpha_l_1*l1_dir_topk(temp_grad, delta.data, X, alpha_l_1, k = 20)
        delta_l_1 = proj_l1ball(delta_l_1, epsilon_l_1, device)
...
def pgd_l1_momentum(model, X,y, epsilon = 12, alpha = 0.01, num_iter = 50, device = "cuda:1"):
    gap = 0.1
    momentum = 0.9
    delta = torch.zeros_like(X, requires_grad = True)
    delta_prev = torch.zeros_like(X, requires_grad = False)
    count = 0
    for t in range (num_iter):
        loss = nn.CrossEntropyLoss()(model(X+delta), y)
        loss.backward()
        ipdb.set_trace()
        delta.data += alpha*l1_dir(delta.grad.detach(), delta.data, X, gap) +delta_prev*momentum
        delta_prev = delta.data
        if (norms_l1(delta) > epsilon).any():
            count += 1
...
def pgd_l1(model, X,y, epsilon = 12, alpha = 0.01, num_iter = 1500, device = "cuda:1"):
    gap = alpha
    delta = torch.zeros_like(X, requires_grad = True)
    # print ("Here")
    count = 0
...
#     neg2 = (grad > 0)*(X_curr == 1)
    ...
    one_hot = one_hot.view(batch_size,channels,32,32)
